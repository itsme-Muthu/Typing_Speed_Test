...
highestWPM = 0;
// ---------------------------------------------------------------------------------------
// Fetch the highest WPM from the server
// ---------------------------------------------------------------------------------------
async function fetchHighestWPM() {
    try {
        const response = await fetch('/api/highest-wpm'); // Endpoint to fetch highest WPM
        if (!response.ok) throw new Error('Network response was not ok');
        const data = await response.json();
        highestWPM = data.highestWPM || 0; // Set the highestWPM variable
        document.querySelector("#highest-wpm").innerText = `Highest WPM: ${highestWPM}`; // Update the HTML to display it
    } catch (error) {
        console.error('Error fetching highest WPM:', error);
    }
}
...
function initTyping(){
    ...
    if(charIndex < characters.length -1 && timeLeft>0){
        ...
        // -----------------------------------------------------------------------------------------------
        // changes made for highest WPM
        // -----------------------------------------------------------------------------------------------
        if (wpm > highestWPM) {
            updateHighestWPM(wpm);
            highestWPM = wpm; // Update local highestWPM variable
            document.querySelector("#highest-wpm").innerText = `Highest WPM: ${highestWPM}`; // Update displayed highest WPM
        }
        inpField.value = "";
        clearInterval(timer);
...
// Function to update the highest WPM in the database
async function updateHighestWPM(newWPM) {
    try {
        const response = await fetch('/api/highest-wpm', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ highestWPM: newWPM }),
        });
        if (!response.ok) throw new Error('Network response was not ok');
        console.log('Highest WPM updated successfully');
    } catch (error) {
        console.error('Error updating highest WPM:', error);
    }
}
...
// Fetch the highest WPM when the page loads
window.onload=()=>{
